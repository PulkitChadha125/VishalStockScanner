...
_HEADER_ALIASES: dict[str, str] = {
    ...
    "entry buffer %": "entry_range_down_pct",
    "entry buffer": "entry_range_down_pct",
    "entry_buffer_pct": "entry_range_down_pct",
    "entry buffer pct": "entry_range_down_pct",
    "entry range down %": "entry_range_down_pct",
    "entry range down": "entry_range_down_pct",
    "entry_range_down_pct": "entry_range_down_pct",
    "entry range up %": "entry_range_up_pct",
    "entry range up": "entry_range_up_pct",
    ...
    for s in symbols:
        writer.writerow(
            [
                ...
                s["stop_loss_pct"],
                s["target_pct"],
                s.get("entry_range_down_pct", s.get("entry_buffer_pct", 2)),
                s.get("entry_range_up_pct", 5),
            ]
        )
    return buf.getvalue()
...
def parse_symbols_csv(file_bytes: bytes) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    if not required.issubset(set(col_map.values())):
        return [], [
            ...
            " (Entry Range Down % defaults to 2, Entry Range Up % defaults to 5)"
        ]
    ...
    for line_no, row in enumerate(rows[1:], start=2):
        ...
        try:
            ...
            down_raw = record.get("entry_range_down_pct")
            up_raw = record.get("entry_range_up_pct")
            if down_raw in (None, ""):
                entry_range_down_pct = 2.0
            else:
                entry_range_down_pct = _parse_number(down_raw)
            if up_raw in (None, ""):
                entry_range_up_pct = 5.0
                if entry_range_down_pct >= entry_range_up_pct:
                    entry_range_up_pct = entry_range_down_pct + 3.0
            else:
                entry_range_up_pct = _parse_number(up_raw)
        except ValueError:
        ...
        if entry_range_down_pct < 0 or entry_range_up_pct < 0:
            errors.append(f"Row {line_no}: entry ranges cannot be negative.")
            continue
        if entry_range_up_pct <= entry_range_down_pct:
            errors.append(
                f"Row {line_no}: entry range up must be greater than entry range down."
            )
            continue
